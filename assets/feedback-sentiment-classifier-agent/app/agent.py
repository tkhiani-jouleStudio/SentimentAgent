...
def get_system_prompt() -> str:
    base_prompt = """You are an AI agent that classifies free-text feedback as positive, negative, or neutral and returns a structured result with sentiment label and confidence score.\n\nYour task is to classify the sentiment of the feedback provided.\n\nRules:\n1. Classify the sentiment as exactly one of: positive, negative, or neutral.\n2. Return ONLY a valid JSON object with two fields: \"sentiment\" (string) and \"confidence\" (float between 0.0 and 1.0).\n3. If confidence is below 0.7, also include \"low_confidence\": true in the JSON.\n4. If the input is empty or blank, return: {\"error\": \"empty_input\", \"message\": \"No feedback text provided.\"}\n5. If you cannot parse a valid classification, return: {\"error\": \"parse_error\", \"message\": \"Could not parse classification result.\"}\n6. Do not include any explanation, prose, or text outside the JSON object.\n\nExample outputs:\n- {\"sentiment\": \"positive\", \"confidence\": 0.95}\n- {\"sentiment\": \"negative\", \"confidence\": 0.88}\n- {\"sentiment\": \"neutral\", \"confidence\": 0.62, \"low_confidence\": true}\n\nIMPORTANT: Never fabricate or invent data. Classify only based on the provided feedback text.""" + _DEFENSIVE_PROMPT_SUFFIX
...
class SampleAgent:
    ...
    async def _classify_sentiment(
        ...
        """Core classification logic with business step instrumentation."""

        # M1 — Feedback Received
        if not query or not query.strip():
            logger.info("M1.missed: feedback input was empty or invalid — classification aborted")
            return '{"error": "empty_input", "message": "No feedback text provided."}'

        with tracer.start_as_current_span("M1_feedback_received"):
            logger.info("M1.achieved: feedback input received and validated")

        # M2 — Sentiment Analysed
        try:
            with tracer.start_as_current_span("M2_sentiment_analysed"):
            ...
            logger.info("M2.achieved: LLM sentiment analysis completed")
        except Exception as exc:
            logger.warning("M2.missed: LLM call failed or returned an invalid response: %s", exc)
            raise
        ...
        try:
            ...
            label = parsed.get("sentiment", "unknown")
            confidence = parsed.get("confidence", 0.0)
            with tracer.start_as_current_span("M3_classification_returned"):
                logger.info(
                    "M3.achieved: classification result returned — label=%s, confidence=%s",
                    label, confidence,
                )
        except Exception:
            logger.warning("M3.missed: classification result could not be formatted or returned")
    ...
    async def stream(
        ...
        try:
            ...
            response = await self._classify_sentiment(query, context_id, tools or [])

            # M4 — Result Delivered
            with tracer.start_as_current_span("M4_result_delivered"):
                logger.info("M4.achieved: result delivered to caller")
            ...
            logger.warning("M4.missed: delivery to caller failed or timed out")
            yield {
                "is_task_complete": True,
                "require_user_input": False,
